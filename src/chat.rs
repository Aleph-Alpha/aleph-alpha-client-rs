...
impl StreamTask for TaskChat<'_> {
    ...
    fn body_to_output(&self, mut response: Self::ResponseBody) -> Self::Output {
        if let Some(usage) = response.usage {
            ...
            StreamChatEvent::Chunk(match chunk {
                DeserializedChatChunk::Delta { delta, logprobs } => ChatChunk::Delta { delta },
                DeserializedChatChunk::Finished { finish_reason } => ChatChunk::Finished {
