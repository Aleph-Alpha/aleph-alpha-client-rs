...
pub use self::{
    chat::{
        ChatOutput, ChatSampling, ChatStreamChunk, Distribution, Message, StreamChatResponse,
        StreamMessage, TaskChat, Usage,
    },
    completion::{
...
impl Client {
    ...
    /// ```no_run
    /// use aleph_alpha_client::{Client, How, TaskChat, Error, Message, ChatStreamChunk};
    /// use futures_util::StreamExt;
    ///
    ...
    ///     while let Some(Ok(event)) = stream.next().await {
    ///         if let ChatStreamChunk::Delta { delta } = event {
